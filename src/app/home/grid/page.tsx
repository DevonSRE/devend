  ...
  return (
    <div className="mt-20">
      ...
      <div className="flex w-full">
        {[
          { src: gridOne, alt: "Grid image 1" },
          { src: gridTwo, alt: "Grid image 2" },
          { src: gridThree, alt: "Grid image 3" },
        ].map(({ src, alt }) => (
          <div key={alt} className="w-1/3 relative aspect-square">
            <div className="absolute top-0 left-0 right-0 bottom-0 bg-black/30 z-10" />
            <Image src={src} alt={alt} className="object-cover w-full h-full" fill sizes="(max-width: 768px) 33vw, 500px" priority />
          </div>
        ))}
      </div>
      ...
      <div className="flex w-full">
        <div className="w-1/3 relative aspect-square">
          <div className="absolute top-0 left-0 right-0 bottom-0 bg-black/30 z-10" />
          <Image src={gridFour} alt="Grid image 4" className="object-cover w-full h-full" fill sizes="(max-width: 768px) 33vw, 500px" />
        </div>
        <div className="w-1/3 relative aspect-square">
          <Image src={gridFive} alt="Grid image 5" className="object-cover w-full h-full" fill sizes="(max-width: 768px) 33vw, 500px" />
          <div className="absolute top-0 left-0 right-0 bottom-0 bg-black/60 z-10" />
          <div className="absolute inset-0 flex items-center justify-center z-20">
            <div>
              <p className="text-white text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-center px-4">DEVEND IN PICTURES</p>
        ...
        <div className="w-1/3 relative aspect-square">
          ...
          <Image src={gridSix} alt="Grid image 6" className="object-cover w-full h-full" fill sizes="(max-width: 768px) 33vw, 500px" />
        </div>
      </div>
      ...
      <div className="flex w-full">
        {[
          { src: gridSeven, alt: "Grid image 7" },
          { src: gridEight, alt: "Grid image 8" },
          { src: gridNine, alt: "Grid image 9" },
        ].map(({ src, alt }) => (
          <div key={alt} className="w-1/3 relative aspect-square">
            <div className="absolute top-0 left-0 right-0 bottom-0 bg-black/30 z-10" />
            <Image src={src} alt={alt} className="object-cover w-full h-full" fill sizes="(max-width: 768px) 33vw, 500px" />
          </div>
        ))}
      </div>
    </div>
