...
const IMAGE_LABEL_MAP: Record<string, string> = {
  ...
  "b1.png": "Notebook - write it down",
  "b2.png": "Notebook - ideas change systems",
...
export default function BrandClient({ categories }: BrandClientProps) {
  ...
  function handleWhatsAppSubmit() {
    if (!form.message.trim()) {
      toast.error("Please write a message before sending to WhatsApp.");
      return;
    }
    ...
    if (form.email.trim()) {
      formattedMessage += `*Email:* ${form.email.trim()}\n`;
    }
    if (formattedMessage) {
      formattedMessage += `\n`;
    }
    ...
    const encoded = encodeURIComponent(formattedMessage.trim());
    const whatsappUrl = `https://wa.me/2347074807461?text=${encoded}`;
                ...
                <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
                  ...
                  <div className="flex flex-col gap-2.5">
                    <button
                      ...
                      className="w-full rounded-xl bg-[#2A1C51] py-3.5 text-sm font-bold text-white transition-all hover:bg-[#3B2259]"
                    >
                    ...
                    <button
                      ...
                      className="w-full rounded-xl bg-[#25D366] py-3.5 text-sm font-bold text-white transition-all hover:bg-[#20ba56] flex items-center justify-center gap-2"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
