...
const ContactForm = () => {
  ...
  const validate = (values: FormValues, checkedItems: CheckedItems): FormErrors => {
  ...
  const handleChange = ({ target }: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (touched[target.name]) {
      setErrors((prev) => ({
        ...prev,
        [target.name]: validate({ ...values, [target.name]: target.value }, checkedItems)[target.name as keyof FormErrors] || "",
      }))
    }
  }
  ...
  const handleBlur = ({ target }: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setErrors((prev) => ({
      ...prev,
      [target.name]: validate(values, checkedItems)[target.name as keyof FormErrors] || "",
    }))
  }
  ...
  const handleDateChange = (date: Date | undefined) => {
    ...
    if (touched.date) {
      setErrors((prev) => ({
        ...prev,
        date: validate({ ...values, date: date ?? "" }, checkedItems).date || "",
      }))
    }
  }
  ...
  const isButtonDisabled = () => {
    ...
    return isLoading || allRequiredFieldsEmpty || !isEventTypeSelected || Object.keys(errors).some((key) => !!errors[key as keyof FormErrors])
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <Input id="firstname" name="firstname" value={values.firstname} onChange={handleChange} onBlur={handleBlur} placeholder="required*" required />
            {errors.firstname && <p className="text-red-500 text-xs">{errors.firstname}</p>}
          </div>
          <div className="space-y-2">
            <Label htmlFor="lastname">Last Name</Label>
            <Input id="lastname" name="lastname" value={values.lastname} onChange={handleChange} onBlur={handleBlur} placeholder="required*" required />
        ...
        <div className="space-y-2">
          ...
          <Input id="email" name="email" type="email" value={values.email} onChange={handleChange} onBlur={handleBlur} placeholder="required*" required />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {eventTypes.map((type) => (
              <div key={type.id} className="flex items-center space-x-2">
                <Checkbox id={`event-${type.id}`} checked={checkedItems[type.id] || false} onCheckedChange={() => handleCheckboxChange(type.id)} />
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="w-full justify-start text-left font-normal">
                {values.date ? format(values.date as Date, "MM/dd/yy") : <span className="text-muted-foreground">mm/dd/yy</span>}
                <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={values.date as Date | undefined} onSelect={handleDateChange} initialFocus />
            </PopoverContent>
          </Popover>
        ...
        <div className="space-y-2">
          ...
          <Input id="needs" name="needs" value={values.needs} onChange={handleChange} onBlur={handleBlur} placeholder="required*" required />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <Input id="budget" name="budget" type="number" value={values.budget} onChange={handleChange} onBlur={handleBlur} placeholder="required*" required />
            {errors.budget && <p className="text-red-500 text-xs">{errors.budget}</p>}
          </div>
          <div className="space-y-2">
            <Label htmlFor="guest">Estimated Guest Count</Label>
            <Input id="guest" name="guest" type="number" value={values.guest} onChange={handleChange} onBlur={handleBlur} placeholder="required*" required />
        ...
        <Button type="submit" disabled={isButtonDisabled()} className="w-full sm:w-auto bg-[#2e1a47] hover:bg-[#3b2259]">
